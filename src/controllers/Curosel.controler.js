...
const curoselImages = asyncHandler(async(req,res)=>{
    ...
    const image1LocalPath = req.file?.image1[0]?.path
    if (!image1LocalPath) {
...
if (!image1) {
    ...
    const image2LocalPath = req.file?.image2[0]?.path
    if (!image2LocalPath) {
...
if (!image2) {
    ...
    const image3LocalPath = req.file?.image3[0]?.path
    if (!image3LocalPath) {
...
const curoselPics = Curosel.create({
    image1 : image1,
    image2 : image2,
    image3 : image3
})
